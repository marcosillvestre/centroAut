...
class WebHookController {
    ...
    async store(req, res) {
        ...
        const {
            md_valor_ex_300_00, //
            md_forma_de_pagamento, //
            md_data_de_pagamento, //
            parcela_forma_de_pagamento,//
            parcela_dia_de_vencimento,//
            tm_data_de_pagamento, //
            data_de_vencimento_da_primeira, data_de_vencimento_da_ultima_p, n_de_parcelas, desconto_total_pontualidade,
            tm_valor_ex_150_00, tm_forma_de_pagamento,
            data_de_emissao_da_venda, valor_da_primeira_parcela, valor_da_parcela_apos_pp,
            nome_do_aluno, data_de_nascimento_do_aluno, background_do_aluno, idade_do_aluno, possui_conhecimento_no_idioma,
            precisa_de_nivelamento, dia_de_aula, professor, horario_de_inicio_ex_9_00, horario_do_fim_ex_10_00,
            classe, subclasse, tipo_modalidade, formato_de_aula, curso, data_da_primeira_aula, n_do_contrato,
            carga_horaria_do_curso, unidade, tipo_de_plano, material_didatico
        } = customFields

        const {
            accountId, name, email, organization, cpf, role, ranking,
            birthday, avatar, createdAt, updatedAt,
            contact, address, category, leadOrigin, author, ownerUser,
            allowedUsers,
        } = person
        ...
        const customerBody = {
            ...
            "person_type": person.cpf.lenght <= 11 ? "LEGAL" : "NATURAL",
            "document": person.cpf,
                ...
                await axios.post('https://api.contaazul.com/v1/customers',
                    ...
                    .catch(async err => {
                        if (err.response.data.message === 'CPF/CPNJ já utilizado por outro cliente.') {
                            await axios.get(`https://api.contaazul.com/v1/customers?document=${person.cpf}`,
